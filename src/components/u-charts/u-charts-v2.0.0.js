...
var util = {
  ...
  isCollision: function isCollision(obj1, obj2) {
    ...
    var flag =
      obj2.start.x > obj1.end.x || obj2.end.x < obj1.start.x || obj2.end.y > obj1.start.y || obj2.start.y < obj1.end.y
    return !flag
  },
}
  ...
  if (opts.legend.position == "top" || opts.legend.position == "bottom") {
    ...
    for (let i = 0; i < series.length; i++) {
      ...
      let itemWidth =
        shapeWidth +
        shapeRight +
        measureText(item.name || "undefined", fontSize, context) +
...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  var seriesConvert = series.map(function(item, index, series) {
    var text = item.formatter
      ? item.formatter(item, index, series)
          ...
          for (let i = 0; i < points.length; i++) {
            ...
            if (item !== null && i > leftNum && i < rightNum) {
              ...
              if (columnOption.meterBorder > 0) {
                ...
                context.lineTo(
                  startX + item.width - columnOption.meterBorder * 0.5,
                  item.y + columnOption.meterBorder * 0.5
                )
    ...
    if (config._xAxisTextAngle_ === 0) {
      newCategories.forEach(function(item, index) {
        ...
        context.fillText(
          ...
          startY + xAxisFontSize + (config.xAxisHeight - scrollHeight - xAxisFontSize) / 2
        )
        context.closePath()
        context.stroke()
      })
      ...
      if (yData.disabled !== true) {
        ...
        if (opts.yAxis.showTitle) {
          ...
          if (yAxisWidth.position == "left") {
            context.fillText(
              title,
              tStartLeft - measureText(title, titleFontSize, context) / 2,
              opts.area[0] - 10 * opts.pix
            )
          } else {
            context.fillText(
              title,
              tStartRight - measureText(title, titleFontSize, context) / 2,
              opts.area[0] - 10 * opts.pix
            )
          }
          context.closePath()
...
function drawGaugeDataPoints(categories, series, opts, config, context) {
  ...
  if (gaugeOption.type == "progress") {
    ...
    let childendX =
      -radius - gaugeOption.width * 0.5 - gaugeOption.splitLine.fixRadius + gaugeOption.splitLine.childWidth
    context.translate(centerPosition.x, centerPosition.y)
    context.rotate((gaugeOption.startAngle - 1) * Math.PI)
    for (let i = 0; i < gaugeOption.splitLine.splitNumber + 1; i++) {
      context.beginPath()
    ...
    if (opts.yAxis.showTitle) {
      ...
      for (let i = 0; i < opts.yAxis.data.length; i++) {
        maxTitleHeight = Math.max(
          maxTitleHeight,
          opts.yAxis.data[i].titleFontSize ? opts.yAxis.data[i].titleFontSize : config.fontSize
        )
      }
